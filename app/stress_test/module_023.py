...
ALLOWED_BASE_DIR_23 = os.path.abspath(os.path.join(os.path.dirname(__file__), "data"))
ALLOWED_URLS_23 = {
...
def read_file_23_2():
    ...
    if not safe_path.startswith(ALLOWED_BASE_DIR_23):
...
def fetch_url_23_4():
    ...
    target = ALLOWED_URLS_23.get(url_key)
    if target is None:
...
def process_23_6():
    ...
    command = ALLOWED_COMMANDS_23.get(cmd)
    if command is None:
